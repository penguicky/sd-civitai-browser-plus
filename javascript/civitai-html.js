...
function select_model(
  ...
  if (event) {
    ...
    if (
      className.includes("custom-checkbox") ||
      className.includes("model-checkbox")
    ) {
      return;
    }
  }
  ...
  if (sendToBrowser) {
    ...
    output = bool
      ? gradioApp().querySelector("#model_sent textarea")
      : gradioApp().querySelector("#model_select textarea");
  }
...
function toggleNSFWContent(hideAndBlur) {
  ...
  dateSections.forEach((section) => {
    ...
    section.style.display =
      !hideAndBlur && cards.length === nsfwCards.length ? "none" : "block";
  });
}
...
function updateCard(modelNameWithSuffix) {
  ...
  if (parentDiv) {
    ...
    cards.forEach((card) => {
      ...
      if (
        onclickAttr &&
        onclickAttr.includes(`select_model('${modelName}', event)`)
      ) {
...
function adjustFilterBoxAndButtons() {
  const element =
    ...
    document.querySelector("#filterBoxL");
  if (!element) return;
  ...
  childDiv.style.marginLeft = isLargeScreen
    ? "0px"
    : isMediumScreen
    ? `${1250 - window.innerWidth}px`
    : "0px";
  element.style.justifyContent =
    isLargeScreen || isMediumScreen ? "center" : "flex-start";
...
function pressRefresh() {
  setTimeout(() => {
    const input = document.querySelector(
      "#pageSlider > div:nth-child(2) > div > input"
    );
...
function updateSVGIcons() {
  ...
  if (childDiv) {
    childDiv.style.cssText = `box-shadow: ${
      isDark ? "#ffffff" : "#000000"
    } 0px 0px 2px 0px; display: none;`;
  }
...
function createTooltip(element, hover_element, insertText) {
  if (element) {
    ...
    tooltip.style.cssText =
      "display: none; text-align: center; white-space: pre;";
...
function setupClickOutsideListener() {
  var filterBox =
    document.getElementById("filterBoxL") ||
...
function createAccordion(containerDiv, subfolders, name, id_name) {
  ...
  toggleButton.innerHTML =
    name +
    '<div style="transition: transform 0.15s; transform: rotate(90deg)">▼</div>';
  toggleButton.onclick = function () {
    accordionDiv.style.display =
      accordionDiv.style.display === "none" ? "block" : "none";
    toggleButton.lastChild.style.transform =
      accordionDiv.style.display === "none" ? "rotate(90deg)" : "rotate(0)";
  };
...
function createCivitAICardButtons() {
  ...
  if (!copyButton) {
    ...
    if (editButton) {
      ...
      const editButtonBeforeStyle = window.getComputedStyle(
        editButton,
        ":before"
      );
    ...
    if (cardDivs.length > 0) {
      ...
      cardDivs.forEach((cardDiv) => {
        ...
        if (!buttonRow.querySelector(".goto-civitbrowser.card-button")) {
          ...
          newDiv.onclick = () =>
            modelInfoPopUp(modelName, cardDiv.parentElement.id);
...
function modelInfoPopUp(
  modelName = null,
  content_type = null,
  no_message = false
) {
  const sendToBrowserElement = gradioApp().querySelector(
    "#setting_civitai_send_to_browser input"
  );
    ...
    for (const button of buttons) {
      if (button.textContent.includes("Browser+")) {
        ...
        const firstButton = document.querySelector(
          "#tab_civitai_interface > div > div > div > button"
        );
        if (firstButton) {
          firstButton.click();
        }
...
function inputHTMLPreviewContent(html_input) {
  ...
  if (startIndex !== -1) {
    ...
    if (endIndex !== -1) {
      ...
      var modelIdNotFound = extractedText.includes(
        ">Model ID not found.<br>The"
      );
...
function metaToTxt2Img(type, element) {
  ...
  const genButton = gradioApp().querySelector(
    "#txt2img_extra_tabs > div > button"
  );
  ...
  const cfg_scale = gradioApp().querySelector(
    "#txt2img_cfg_scale > div:nth-child(2) > div > input"
  );
  let final = "";
...
function multi_model_select(modelName, modelType, isChecked) {
  ...
  const selected_model_list = gradioApp().querySelector(
    "#selected_model_list textarea"
  );
  selected_model_list.value = JSON.stringify(selectedModels);

  const selected_type_list = gradioApp().querySelector(
    "#selected_type_list textarea"
  );
...
function setSortable() {
  new Sortable(document.getElementById("queue_list"), {
    onEnd: function (evt) {
      const gradio_input = document.querySelector(
        "#civitai_dl_list.prose"
      ).innerHTML;
      const gradio_html = gradioApp().querySelector(
        "#queue_html_input textarea"
      );
...
function setDownloadProgressBar() {
  ...
  const interval = setInterval(() => {
    ...
    if (percentage >= 100) {
      ...
      dlBtn.setAttribute(
        "onclick",
        "removeDlItem(" + parseInt(dlId) + ", this)"
      );
    ...
    if (currentDlCancelled) {
      ...
      dlText.includes("Encountered an error during download of") ||
      dlText.includes("not found on CivitAI servers") ||
      dlText.includes("requires a personal CivitAI API to be downloaded")
    ) {
      clearInterval(interval);
      dlBtn = dlItem.querySelector(".dl_action_btn > span");
      dlBtn.innerText = "Remove";
...
function hideInstalled(toggleValue) {
  const modelList = document.querySelectorAll(
    ".column.civmodellist > .civmodelcardinstalled"
  );
...
function createCustomSubfolder(subfolderDiv, subfolderId, subfolderValue) {
  ...
  newContainerDiv.classList.add(
    "svelte-1f354aw",
    "container",
    "CivitDefaultSubfolder"
  );
...
function createSubfolderButton() {
  ...
  const customSubfoldersList = document.querySelector(
    "#custom_subfolders_list"
  );
  ...
  plusButton.addEventListener("click", function () {
    const existingSubfolderDivs = document.querySelectorAll(
      "div.CivitDefaultSubfolder"
    );
  ...
  optionsText.textContent =
    "Available options: {BASEMODEL} {NSFW} {AUTHOR} {MODELNAME} {MODELID} {VERSIONNAME} {VERSIONID}";
...
function onPageLoad() {
  ...
  let subfolderDiv = document.querySelector(
    "#settings_civitai_browser_plus > div > div"
  );
  let downloadDiv = document.querySelector(
    "#settings_civitai_browser_download > div > div"
  );
  let settingsDiv = document.querySelector(
    "#settings_civitai_browser > div > div"
  );
  ...
  if (subfolderDiv || downloadDiv) {
    ...
    createAccordion(
      ...
      subfolders,
      "Default sub folders",
      "default-sub-accordion"
    );
    createAccordion(
      div,
      null,
      "Create sub folder entries",
      "create-sub-accordion"
    );
